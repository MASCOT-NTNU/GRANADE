...
# Path to the synched data
path_auv_data = "/Users/ajolaise/Library/CloudStorage/OneDrive-NTNU/PhD/code/Phoenix/auv_data/synched_data/20230622.csv"

csv_data = pd.read_csv(path_auv_data, sep=",")

# Loading the data
salinity = np.array(csv_data["salinity"])
T = np.array(csv_data["timestamp"])
lat = np.array(csv_data["lat"])
lon = np.array(csv_data["lon"])
y,x = WGS.latlon2xy(lat, lon)
S = np.stack((x,y),axis=1)
print(S.shape)
depth = np.array(csv_data["depth"])
...
AUV_data.add_new_datapoints(S, T, salinity, depth)
...
if dashboard_type == "full":
    ...
    plt.savefig("src/plots/dashboard/dashboard_"+ "Post" + '.png', dpi=150)
    plt.close()
